...
//   //const aspectRatios = [1.314, 1.33, 0.75, 0.747, 1.483, 1.446];
...
//   // Disable ALL background scroll when modal open (no double scrollbars)
//   useEffect(() => {
...
//   // Header scroll logic (same as Home)
...
//     md:h-auto md:aspect-[1800/1402]   /* md & above → original aspect ratio */
//   `}
//         onClick={(e) => {
...
//         }}
//       >
//         <img src={`http://localhost:5000/${project.images[0]}`} alt={project.title} className="absolute inset-0 w-full h-full object-cover" draggable={false} />
...
//       {/* INFO SECTION */}
//       <div className="container mx-auto px-6 md:px-10 py-14 bg-[#f4f3f1]">
//         <div className="grid md:grid-cols-2 gap-12 items-center">
//           {/* LEFT SIDE */}
//           <div className="flex flex-col justify-center items-center text-center gap-6 md:pl-6">
//             {/* Title */}
//             <div>
//               <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2 leading-snug">{project.title}</h2>
//             </div>

//             {/* Contact Description (displayed top to bottom) */}
...
//               {project.pdfFile && (
//                 <a href={`http://localhost:5000/${project.pdfFile}`} download className="inline-flex items-center gap-2 text-gray-700 hover:text-black font-medium transition">
//                   <FileDown className="w-5 h-5" />
//                   <span>Download PDF</span>
//                 </a>
//               )}

//               {/* Get in Touch button next to Download */}
...
//         <div className="px-5 py-5 grid grid-cols-2 md:grid-cols-4 gap-5">
//           {project.images.slice(1, 31).map((img, idx) => {
//             // Extract file name
//             const fileName = img.split("/").pop() || "";
//             const parts = fileName.split(".");

//             // Combine parts[1] and parts[2] → "1.333" or "0.750"
...
//           `}
//               >
//                 <img src={`http://localhost:5000/${img}`} alt={`${project.title} ${idx}`} className="absolute inset-0 w-full h-full object-cover" draggable={false} />
//               </div>
//             );
//           })}
//         </div>
...
//           {/* Zoom Controls */}
//           <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex gap-4 text-white">
//             <button onClick={() => setZoom((z) => Math.max(z - 0.2, 1))} className="text-3xl font-bold px-3 pb-1 rounded-full bg-gray-700 bg-opacity-60">
//               −
//             </button>
//             <button onClick={() => setZoom((z) => Math.min(z + 0.2, 3))} className="text-3xl font-bold px-3 pb-1 rounded-full bg-gray-700 bg-opacity-60">
//               +
//             </button>
//           </div>
//         </div>
      ...
      {selectedIndex !== null && (
        ...
        >
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-4 text-white">
            <button onClick={() => setZoom((z) => Math.max(z - 0.2, 1))} className="text-3xl px-3 pb-1 bg-gray-700 bg-opacity-60 rounded-full">
              −
            </button>
            <button onClick={() => setZoom((z) => Math.min(z + 0.2, 3))} className="text-3xl px-3 pb-1 bg-gray-700 bg-opacity-60 rounded-full">
              +
            </button>
          </div>
        </div>
...
{
  /* <div className="w-full bg-white">
        <div className="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-[30px]">
          {project.images.slice(1, 31).map((img, idx) => (
            <div key={idx} className={`cursor-pointer aspect-[${aspectRatios[idx]}]`} onClick={() => setSelectedIndex(idx + 1)}>
              <img src={`http://localhost:5000/${img}`} alt={`${project.title} ${idx}`} className={`w-full  object-cover transition-all hover:opacity-90`} draggable={false} />
            </div>
          ))}
        </div>
      </div> */
...
{
  /* <div className="w-full bg-white">
        <div className="px-5 py-5 grid grid-cols-1 md:grid-cols-4 gap-6">
          {project.images.slice(1, 31).map((img, idx) => {
